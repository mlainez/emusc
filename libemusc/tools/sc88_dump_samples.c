 ...
 *
 * The library's own descramble and FCE decoder are used rather than a
 * reimplementation, so what lands on disk is exactly what the renderer
 * plays. Names come from the multisample directory that references each
 * sample, since that is the only name the ROM gives them.
 */
#include "sc88_rom.h"
#include "sc88_wave.h"
 ...
 * sample is sustained by following its own loop for `seconds` and faded at
 * the end so the file does not click. One-shots are left exactly as they
 * are. The loop point is `address_b` and the end `address_c`, both
 * relative to the decoded base.
 */
static int32_t *sustain(const int32_t *pcm, size_t count, uint32_t base,
                        const struct sc88_wave_descriptor *desc,
                        enum sc88_wave_loop_type loop, unsigned rate,
                        double seconds, size_t *out_count)
{
  size_t loop_start = desc->address_b > base
    ? (size_t)(desc->address_b - base) : 0u;
  size_t loop_end = desc->address_c > base
    ? (size_t)(desc->address_c - base) : 0u;
  size_t target = (size_t)(seconds * rate);
  size_t span, filled, fade, i;
  int32_t *out;
  int forward = 1;

  if (loop_end > count)
    loop_end = count;
  if (loop_start >= loop_end || count < 2 ||
      loop == SC88_WAVE_FORWARD_ONE_SHOT ||
      loop == SC88_WAVE_REVERSE_ONE_SHOT || count >= target) {
    out = (int32_t *)malloc(count * sizeof *out);
    if (!out)
      return NULL;
    memcpy(out, pcm, count * sizeof *out);
    *out_count = count;
    return out;
  }
  span = loop_end - loop_start;
  out = (int32_t *)malloc(target * sizeof *out);
  if (!out)
    return NULL;
  memcpy(out, pcm, loop_end * sizeof *out);
  filled = loop_end;
  while (filled < target) {
    size_t take = target - filled < span ? target - filled : span;
    if (loop == SC88_WAVE_PING_PONG_LOOP && !forward) {
      for (i = 0; i < take; ++i)
        out[filled + i] = pcm[loop_end - 1 - i];
    } else {
      memcpy(out + filled, pcm + loop_start, take * sizeof *out);
    }
    filled += take;
    if (loop == SC88_WAVE_PING_PONG_LOOP)
      forward = !forward;
  }
  ...
  for (i = 0; i < fade; ++i) {
    ...
    out[target - fade + i] = (int32_t)((double)out[target - fade + i] * g);
  }
  *out_count = target;
  return out;
}
...
int main(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "--control") && i + 1 < argc)
      ...
      fprintf(stderr, "usage: %s --control FILE --wave A B C D "
              "[--out-dir DIR]\n", argv[0]);
      return 2;
    }
  }
      ...
      if (pointer != 0xffff &&
        ...
        if (pcm && desc.address_c > base &&
          ...
          loop_name =
            ...
            : "reverse-one-shot";
          played = sustain(pcm, count, decoded_base, &desc, loop,
                           NATIVE_RATE, loop_seconds, &played_count);
          ...
          if (played && write_wav(path, played, played_count, NATIVE_RATE)) {
            fprintf(index,
                    ...
                    "0x%06x\t0x%06x\t0x%06x\t0x%04x\t%.4f\n",
                    (unsigned)descriptor_offset, name, (unsigned)boundary,
                    ...
                    (double)played_count / NATIVE_RATE);
            ++written;
          } else {
            ++refused;
          }
