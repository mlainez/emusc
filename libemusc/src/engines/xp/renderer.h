...
struct xp_renderer {
  ...
  xp_tvf_audio_transfer_fn tvf_audio_transfer;
  void *tvf_audio_user;
...
static inline bool xp_renderer_send_gain(
  const struct xp_renderer *renderer, uint8_t control, float *gain)
{
  if (control > 127)
    return false;
...
struct xp_render_component {
  ...
  /* The composed amplitude is handed to the XP chip as a TARGET with an
     ...
     way, that `xp_tvf_registers` uses for TVF-F.
  ...
  struct xp_tvf_audio_state tvf_audio;
...
/* tva_curve_decode's result for XP_STATIC_AMPLITUDE_CURVE_WORD never
   ...
   xp_render_static_gain_q17 below so a caller whose voices share one
   ...
   1.0), which xp_render_static_gain_q17_from_progress below turns
   back into component->static_gain_current_q17 unchanged, so no
   separate period_fraction <= 0.0 case is needed here. */
static inline double xp_static_gain_progress(double period_fraction)
{
  const double q = kXpStaticAmplitudeRate * period_fraction;
  return q >= 40.0 ? 1.0 : 1.0 - std::exp(-q);
}

static inline uint32_t xp_render_static_gain_q17_from_progress(
  const struct xp_render_component *component, double progress)
{
...
static inline uint32_t xp_render_static_gain_q17(
  const struct xp_render_component *component, double period_fraction)
{
  if (period_fraction <= 0.0)
    return component->static_gain_current_q17;
  return xp_render_static_gain_q17_from_progress(
    component, xp_static_gain_progress(period_fraction));
}

struct xp_render_voice {
  struct xp_render_component components[XP_MAX_TONE_COMPONENTS];
...
// embeds xp_render_component and xp_render_voice by value through
// engine.h, and with xp_renderer_test.c, which reads their fields
// directly.
//
...
// xp_render_component fields the per-sample render loop reads, and both
