...
protected:
  ...
  // Only _iterate_phase()'s normal-duration branch ever assigns this; a
  // phase whose very first tick takes the short-duration branch instead
  // (duration <= _instantTicks) leaves it unset for the note's lifetime.
  int _phaseStepSize = 0;          // Increase phase duration
