...
private:
  ...
  const _CPUMemoryMapLUT SC55mkII_1_01_CPU_LUT {
    // LFORate was 0x6486, which breaks the 0x100 ladder its neighbours sit
    // on and points at bytes whose values all exceed the rate clamp, so
    // every tone's LFO ran pinned at 20 Hz instead of its own rate. The
    // table at 0x6d86 is monotone, its top entry is the clamp constant
    // itself, and it sits the same distance past the envelope-time table
    // as the SC-55's does (PROVENANCE.md P-0112, P-0113).
