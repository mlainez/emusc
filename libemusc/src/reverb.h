...
//
// The JV-880 runs the SAME program on the same chip with its own numbers. Its
// firmware streams each reverb type's 30-word ROM2 record into exactly the
// registers the character blocks below fill - words 0-11 to p28, 12-20 to p29,
// 22-26 to c4..c8, 21 to the pre-LPF - and three of its six records are
// byte-identical in their pointer words to blocks below (ROOM2 = Plate,
// HALL2 = Room2, STAGE2 = Hall2). Selected by ReverbNetworkKind::
// JVRecordProgram, the loader in reverb.cc reads those records in place of a
// block here; nothing else about the program changes. Measured against the
// reference on first-arrival timing per channel, sign, level, loop-mode
// positions and decay; see _set_jv_character.
...
private:
  ...
  // The two bytes of a JV-880 reverb record that the parameter path, not the
  // streamed copy, reads (ControlRom::LookupTables::JVReverbRecord): the delay
  // arm's base address, word 13 + 1, and the Time scale at +0x36. The record's
  // other 27 words go straight into _activeCharRegs.
  uint16_t _jvTapBase;
  int      _jvTimeScale;
  ...
  // Whether this device's registers come from its own ROM records rather than
