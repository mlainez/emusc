...
int main()
{
  ...
  assert(pitch_envelope_prepare(&rom, &tone, &component, 60, 64,
                                &envelope));
  ...
  assert(pitch_envelope_advance(&envelope, 1));
  assert(envelope.current == 0x06ff);
  assert(pitch_release_prepare(&rom, &tone, &component, 60,
  ...
  envelope.stage = 4;
  assert(pitch_release_advance(&release, 1));
  assert(pitch_envelope_sum(&envelope, &release) < envelope.current);
  ...
  assert(pitch_current_word(0x38000, 0, 0) == 0x38000);
  assert(pitch_current_word(0x38000, 0, 0x100) == 0x38200);
  assert(pitch_current_word(0x38000, 0x4000, 0) == 0x3c000);
  assert(pitch_current_word(0x38000, -0x4000, 0) == 0x34000);
  /* the part offset and the doubled envelope add in the same domain */
  assert(pitch_current_word(0x38000, 0x1000, 0x800) == 0x3a000);
  /* an odd result is rounded down, never up */
  assert(pitch_current_word(0x38001, 0, 0) == 0x38000);
  assert(pitch_current_word(0x38003, 0, 0) == 0x38002);
  ...
  assert(pitch_current_word(0x3fffe, 0, 0x4000) == 0x3fffe);
  assert(pitch_current_word(0x3ffff, 0, 0) == 0x3fffe);
  assert(pitch_current_word(0x100, -0x4000, 0) == 0x3fffe);
  assert(pitch_current_word(0, 0, -0x4000) == 0x3fffe);
  ...
  assert(pitch_current_word(0x40000, 0, 0) == 0x3fffe);
  assert(pitch_current_word(1, -1, 0) == 0);
  assert(pitch_current_word(0, -1, 0) == 0x3fffe);
     ...
     the top of the range. */
  {
    ...
    assert(pitch_current_word(low_base, full_bend_down,
  ...
  assert(portamento_rate(&rom, 1) == 0x00123456u);
  assert(portamento_rate(&rom, 70) == 0x00010000u);
...

  {
    ...
    glide.active = true;
    portamento_advance(&glide, 4);
    assert(glide.current == 74u << 16);
    portamento_advance(&glide, 1000);
    assert(glide.current == glide.target);
    assert(!glide.active);
