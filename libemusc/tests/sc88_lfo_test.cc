...
int main()
{
  ...
  assert(lfo_rate_control(4000, &control) && control == 5243);
  assert(lfo_rate_control(32767, &control) && control == 5243);
  assert(lfo_rate_control(-4000, &control) && control == -5244);
  assert(lfo_rate_control(-32768, &control) && control == -5244);
  assert(lfo_rate_control(0, &control) && control == 0);
  ...
  assert(lfo_common_rate_index(60, 64, 64, &index) && index == 60);
  assert(lfo_common_rate_index(60, 127, 127, &index) && index == 127);
  assert(lfo_common_rate_index(60, 0, 0, &index) && index == 0);
  assert(!lfo_common_rate_index(128, 64, 64, &index));
  assert(lfo_common_delay_index(40, 64, 64, &delay_index) &&
         delay_index == 40);
  /* a negative delay byte bypasses the table instead of indexing it */
  assert(lfo_common_delay_index(-1, 64, 64, &delay_index) &&
         delay_index == -1);
  ...
  assert(lfo_effective_increment(0x1000, 0x100, &phase) &&
         phase == 0x1100);
  assert(lfo_effective_increment(0x0100, -0x0100, &phase) && phase == 0);
  assert(lfo_effective_increment(0x0100, -0x0200, &phase) && phase == 0);
  assert(lfo_effective_increment(0x28f6, 1, &phase) && phase == 0x28f6);
  assert(lfo_effective_increment(0xffff, 2, &phase) && phase == 1);

  /* The procedural shapes, at the phases that define them. */
  assert(lfo_square(0) == 32767 && lfo_square(0x8000) == -32768);
  assert(lfo_triangle(0) == 0);
  assert(lfo_triangle(0x4000) == 32767);
  assert(lfo_triangle(0xc000) == -32768);
  assert(lfo_rectified_triangle(0) == 0);
  assert(lfo_rectified_triangle(0x4000) == 32767);
  /* the negative half is shifted by -32767 modulo 65536, not rectified */
  assert(lfo_rectified_triangle(0xc000) ==
         (int16_t)((uint16_t)((uint16_t)-32768 - (uint16_t)0x7fff)));

  /* The random word is a byte swap of seed plus phase. */
  assert(lfo_random_target(0x0000, 0x1234) == 0x3412);
  assert(lfo_random_target(0x00ff, 0x0001) == 0x0001);
  /* and the slew moves by at most 0x1c2, stopping exactly on the target */
  assert(lfo_slew_random(0, 32767) == 0x1c2);
  assert(lfo_slew_random(0, -32768) == -0x1c2);
  assert(lfo_slew_random(100, 150) == 150);
  assert(lfo_slew_random(-100, -150) == -150);
  ...
  seed = 0;
  target = 0;
  assert(lfo_phase_advance(0x1000, 0, &phase, &seed, &target) &&
         phase == 0x1000 && seed == 0 && target == 0);
  assert(lfo_phase_advance(0x1000, 3, &phase, &seed, &target) &&
         phase == 0x5000);
  phase = 0x7fff;
  seed = 0x1111;
  assert(lfo_phase_advance(1, 0, &phase, &seed, &target) &&
         phase == 0x8000);
  assert(seed == lfo_random_target(0x1111, 0x8000) && target == seed);
  assert(!lfo_phase_advance(0, 0, &phase, &seed, &target));
  assert(!lfo_phase_advance(0x28f7, 0, &phase, &seed, &target));
  ...
  assert(lfo_table_sample(&rom, SINE_TABLE, 0x0100, 0x0200, &out) &&
         out == 0);
  assert(lfo_table_sample(&rom, SINE_TABLE, 0x0100, 0x01ff, &out) &&
         out == 128);
  assert(lfo_waveform(&rom, 0x00, 0x0100, 0x01ff, 0, 0, &out) &&
         out == 128);
  /* every unassigned dispatch entry returns the phase word itself */
  assert(lfo_waveform(&rom, 0x04, 0x1234, 0x0100, 0, 0, &out) &&
         out == 0x1234);
  assert(lfo_waveform(&rom, 0x1e, 0x1234, 0x0100, 0, 0, &out) &&
         out == 0x1234);
  assert(lfo_waveform(&rom, 0x0a, 0, 0x0100, 0, 4321, &out) &&
         out == 4321);
  assert(!lfo_waveform(&rom, 0x01, 0, 0x0100, 0, 0, &out));
  assert(!lfo_waveform(&rom, 0x20, 0, 0x0100, 0, 0, &out));
  ...
  assert(lfo_ramp_initialize(0x4000, 0x0010, &ramp) &&
         ramp.fade == 0 && ramp.delay_phase == 0x4000);
  assert(lfo_ramp_advance(&ramp, 0) && ramp.fade == 0);
  assert(lfo_ramp_advance(&ramp, 0) && ramp.fade == 0);
  /* the step whose delay addition carries is the one that begins the fade */
  assert(lfo_ramp_advance(&ramp, 0) && ramp.fade == 0x0010);
  /* and catch-up spends the missed periods on the fade once it has begun */
  assert(lfo_ramp_initialize(0x4000, 0x0010, &ramp));
  assert(lfo_ramp_advance(&ramp, 4) && ramp.fade == 0x0030);
  assert(lfo_ramp_initialize(UINT16_MAX, 0x0010, &ramp) &&
         ramp.fade == 0x0010);
  assert(lfo_ramp_initialize(0, 0x0010, &ramp) && ramp.fade == 0);
  assert(lfo_ramp_activate_immediate(&ramp) && ramp.fade == 1);
  /* and the fade saturates on carry rather than wrapping to silence */
  assert(lfo_ramp_initialize(UINT16_MAX, 0x8000, &ramp));
  assert(lfo_ramp_advance(&ramp, 4) && ramp.fade == UINT16_MAX);
  ...
  assert(lfo_common_prepare(&rom, &tone, 64, 64, 64, 64, &lfo));
  ...
  assert(lfo_local_prepare(&rom, &component, &lfo));
  ...
  seed = 0;
  assert(lfo_advance(&rom, &lfo, 0, 0, &seed));
  assert(lfo.phase == (uint16_t)(0x8000 + 0x0300));
  assert(lfo.output == lfo_square(lfo.phase));
  /* a contribution that cancels the base stalls it: phase and output stand */
  phase = lfo.phase;
  out = lfo.output;
  assert(lfo_advance(&rom, &lfo, -0x0300, 0, &seed));
  assert(lfo.phase == phase && lfo.output == out);

  /* The frequency the recovered timer gives each increment. */
  assert(fabs(lfo_frequency(0) - 0.0) < 1e-12);
  assert(fabs(lfo_frequency(0x0034) - 0.099172) < 5e-7);
  assert(fabs(lfo_frequency(0x28f6) - 19.998458) < 5e-7);
