...
int main(void)
{
  ...
  assert(sc88_pan_pair_q15(&rom, 1, &left, &right));
  assert(left == 0x0000 && right == 0x8000);
  assert(sc88_pan_pair_q15(&rom, 127, &left, &right));
  assert(left == 0x8000 && right == 0x0000);
  /* out of range on both sides */
  assert(!sc88_pan_pair_q15(&rom, 0, &left, &right));
  assert(!sc88_pan_pair_q15(&rom, 128, &left, &right));
  ...
  assert(!sc88_pan_pair_q15(&rom, 64, &left, &right));
  ...
  assert(sc88_control_gain_q15(&rom, 0, &gain) && gain == 0x0000);
  assert(sc88_control_gain_q15(&rom, 127, &gain) && gain == 0x8000);
  put16(bytes + SEND_TABLE + 5 * 2, 0x1234);
  assert(!sc88_control_gain_q15(&rom, 5, &gain));
  ...
  assert(sc88_send_combine(0, 0) == 0);
  assert(sc88_send_combine(127, 127) == 127);
  assert(sc88_send_combine(64, 127) == 64);
  assert(sc88_send_combine(1, 127) == 1);
     ...
     offset is the component's own byte plus the ROM's per-key byte. */
  {
    ...
    assert(sc88_pan_component_offset(&rom, &tone, &component, 5, &offset));
    assert(offset == 7);
    assert(!sc88_pan_component_offset(&rom, &tone, &component, 200, &offset));
    ...
    put16(bytes + PAN_TABLE + (67 - 1) * 2, 0x2000);
    {
      ...
      assert(sc88_pan_static_q15(&rom, &tone, &component, 5, &controls,
                                 &position, &left, &right));
