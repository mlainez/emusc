...
int main()
{
  ...
  /* pan_pair_q15: position 1 reads left from the table's LAST entry
  ...
  /* the low six bits are a destination selector this engine doesn't plumb;
     a dirty one is reported false, and (unlike control_gain_q15) the
  ...
  /* control_gain_q15: control 0 is silence, 127 unity, and the send
  ...
  /* send_combine: the rounded product maps 0 to 0 and 127x127 to 127,
  ...
  /* pan_component_offset: the key-table pointer is a 24-bit address
     ...
     offset is the component's own byte plus the ROM's per-key byte. */
  {
    ...
    /* pan_static_q15: composition clamps to 1..127, and a part pan of
    ...
    {
      struct xp_pan_controls controls;
      controls.master = 64;
      controls.part = 60;
